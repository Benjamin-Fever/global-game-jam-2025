...
public partial class GameMap : TileMapLayer {
	...
	public override void _Ready() {
		...
		astar.Offset = (Vector2)TileSet.TileSize / 2 + Vector2.One * 2;
		...
		for (int x = GetUsedRect().Position.X; x < tileLayerSize.X; x++) {
			for (int y = GetUsedRect().Position.Y; y < tileLayerSize.Y; y++) {
